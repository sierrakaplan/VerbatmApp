...
-(void)addImage: (id) givenImageView withPinchObject: (PinchView *) pinchObject
{
    
    if([givenImageView isKindOfClass:[VerbatmImageView class]])
    {
        if(((VerbatmImageView*)givenImageView).isVideo)
        {
            self.openImage = [[VerbatmImageView alloc]init];
            [self addSubview:self.openImage];
            CGRect frame = self.bounds;
            self.openImage.frame = frame;
            AVURLAsset* asset = [[AVURLAsset alloc]initWithURL:((VerbatmImageView*)givenImageView).asset.defaultRepresentation.url options:nil];
        ...
        }else
        {
            ...
            self.openImage = [[VerbatmImageView alloc]init];
            self.openImage.image = ((VerbatmImageView*)givenImageView).image;
            self.openImage.asset = ((VerbatmImageView*)givenImageView).asset;
