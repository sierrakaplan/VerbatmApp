...
#define X_OFFSET 7.f
#define FOLLOW_BUTTON_X_POS (self.frame.size.width - X_OFFSET - LARGE_FOLLOW_BUTTON_WIDTH)

#define POST_TEXT_WIDTH 50.f
...
-(void)presentNotification:(NotificationType) notificationType withChannel:(Channel *) channel andParseObject:(PFObject *)parseObject{
	...
	CGFloat notificationTextWidth = self.frame.size.width - (X_OFFSET*2);
	if(notificationType & NotificationTypeLike) {
		[self createHeartIcon];
		notificationTextWidth -= (LARGE_FOLLOW_BUTTON_HEIGHT + FOLLOW_TEXT_BUTTON_GAP + X_OFFSET);
		...
		notificationTextWidth -= (LARGE_FOLLOW_BUTTON_WIDTH + FOLLOW_TEXT_BUTTON_GAP);
	}
	if(notificationType & NOTIFICATION_TYPE_WITH_POST) {
		notificationTextWidth -= (POST_TEXT_WIDTH + FOLLOW_TEXT_BUTTON_GAP);
	}
	...
	[self createNotificationLabelWithAttributedString: notificationLabel andWidth: notificationTextWidth];
	if(notificationType & NOTIFICATION_TYPE_WITH_POST) {
...
-(void)createNotificationLabelWithAttributedString:(NSAttributedString *) notification andWidth:(CGFloat)labelWidth {
	CGFloat yposition = self.frame.size.height/2.f - LARGE_FOLLOW_BUTTON_HEIGHT/2.f;
	CGRect frame = CGRectMake(X_OFFSET, yposition,
	...
	self.notificationTextLabel.baselineAdjustment = UIBaselineAdjustmentNone;
...
-(void)createHeartIcon {
	CGFloat x_pos = self.frame.size.width - X_OFFSET*2 - LARGE_FOLLOW_BUTTON_HEIGHT;
